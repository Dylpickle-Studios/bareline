...
| Second-factor bypass/replay (TOTP)                   | `totp-service.test.ts`, `totp-login-flow.integration.test.ts`, `security-regression.integration.test.ts`, `base32.test.ts`        |
| Issue tracker authorization and content sanitization | `issue-service.test.ts`, `issues.integration.test.ts`, `search-issues.integration.test.ts`, `markdown.test.ts` (shared sanitizer) |
